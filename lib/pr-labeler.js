...
module.exports = class PullRequestLabeler {
  ...
  async label() {
    ...
    if ("head-branch-labels" in this.config) {
      this.labelHeadBranch(this.config["head-branch-labels"])
    }
  ...
  labelHeadBranch(branchesConfig) {
    this.log.info("Head branch is", this.pull.head.ref)

    for (const label in branchesConfig) {
      this.allLabels.add(label)

      this.log.debug("Looking for branch", label, branchesConfig[label])
      const matcher = ignore().add(branchesConfig[label])
      
      if (matcher.ignores(this.pull.head.ref)) {
        this.log.info("Found branch, applying label", label, branchesConfig[label])
        this.newLabels.add(label)
      }
    }
  }
